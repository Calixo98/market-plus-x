...
module.exports = async (req, res) => {
  ...
  if (req.method !== 'POST') return res.status(405).json({ ok: false }); let body = req.body; if (typeof body === 'string') { try { body = JSON.parse(body); } catch { return res.status(400).json({ ok: false, error: 'JSON invalido' }); } } const text = typeof body?.body === 'string' ? body.body.trim() : '';
