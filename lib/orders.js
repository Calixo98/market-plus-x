const crypto = require('crypto'); const kv = require('./kv'); const catalogo = require('./catalogo'); const { notifyOrder } = require('./email');
...
async function createCodOrder(input) {
  ...
  try {
    ...
    await kv.set(`pedido:${referencia}`, JSON.stringify(order)); return order;
...
async function updateCodOrder(reference, action) {
  ...
  return kv.withLock(`lock:${key}`, async () => { const raw = await kv.get(key); if (!raw) throw new Error('Pedido no encontrado'); const order = JSON.parse(raw);
    if (action === 'confirm' && order.estado === 'COD_PENDING_CONFIRMATION') { order.estado = 'COD_CONFIRMED'; order.confirmadoEn = new Date().toISOString(); for (const item of order.items) await kv.incrby(`vendido:${item.sku}`, item.qty); }
    else if (action === 'cancel' && order.estado === 'COD_PENDING_CONFIRMATION' && !order.stockLiberado) { await catalogo.liberarStock(order.items); order.estado = 'COD_CANCELLED'; order.stockLiberado = true; order.canceladoEn = new Date().toISOString(); }
    await kv.set(key, JSON.stringify(order)); return order;
  });
}
