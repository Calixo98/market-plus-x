...
async function updateCodOrder(reference, action) {
  ...
  return kv.withLock(`lock:${key}`, async () => { const raw = await kv.get(key); if (!raw) throw new Error('Pedido no encontrado'); const order = JSON.parse(raw);
    let justConfirmed = false; let shouldEmitPurchase = false; let justCancelled = false;
    if (action === 'confirm' && order.estado === 'COD_PENDING_CONFIRMATION') { order.estado = 'COD_CONFIRMED'; order.confirmadoEn = new Date().toISOString(); for (const item of order.items) await kv.incrby(`vendido:${item.sku}`, item.qty); justConfirmed = true; shouldEmitPurchase = true; }
      ...
      const reserved = await catalogo.reservarStock(order.items.map(item => ({ sku: item.sku, qty: item.qty })));
      if (!reserved.ok) throw new Error(`Sin stock suficiente para ${reserved.sku}`);
      const now = new Date().toISOString();
      const wasPreviouslyConfirmed = Boolean(order.confirmadoEn);
      order.estado = 'COD_CONFIRMED'; order.stockLiberado = false; order.expiraEn = null; order.canceladoEn = null; order.confirmadoEn = order.confirmadoEn || now; order.reactivadoEn = now;
      if (!wasPreviouslyConfirmed) { for (const item of order.items) await kv.incrby(`vendido:${item.sku}`, item.qty); shouldEmitPurchase = true; }
      justConfirmed = true;
    }
    else if (action === 'cancel' && ['COD_PENDING_CONFIRMATION', 'COD_CONFIRMED'].includes(order.estado) && !order.stockLiberado) { await catalogo.liberarStock(order.items); order.estado = 'COD_CANCELLED'; order.stockLiberado = true; order.canceladoEn = new Date().toISOString(); justCancelled = true; }
    await kv.set(key, JSON.stringify(order));
    if (shouldEmitPurchase) await enviarEventoCompra({ referencia:`cod-purchase-${order.referencia}`, total:order.total, moneda:'COP', email:order.cliente.email, telefono:order.cliente.telefono, eventTime:Math.floor(Date.now()/1000) });
