...
// https://interrapidisimo.com/wp-content/uploads/TARIFAS-INTER-RAPIDISIMO.pdf
...
function calcularEnvio({ ciudad, items = [], metodoPago = 'bold', subtotal, linea } = {}) {
  ...
  const base = Number(zona.tarifaRacingBase) * Math.max(1, unidadesRacing);
...
function estimarEnvio({ ciudad, linea = 'racing' } = {}) {
  ...
  return {
    ...
    paquete,
    nota: linea === 'racing'
      ? `Promedio para un carro del catálogo e incluye ${envios.racing.pagoEnCasaPorcentaje}% de Pago en Casa. ${envios.racing.nota}`
