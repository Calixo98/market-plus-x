...
async function rateLimit(key, max, seconds) {
  ...
  const count = await kv.evalScript("local current = redis.call('incr', KEYS[1]); if current == 1 then redis.call('expire', KEYS[1], ARGV[1]); end; return current", [redisKey], [seconds]);
...
async function verifyTurnstile(token, req) {
  ...
  const response = await fetchWithTimeout('https://challenges.cloudflare.com/turnstile/v0/siteverify', { method: 'POST', body: new URLSearchParams({ secret, response: token, remoteip: ipOf(req) }) }, 4000);
