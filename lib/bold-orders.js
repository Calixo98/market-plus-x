...
async function processExistingOrder(orderKey, existing, event, key) {
...
async function processBoldEvent(reference, event) {
  ...
  return kv.withLock(`lock:${reservationKey}`, async () => {
    ...
    if (existing) return processExistingOrder(orderKey, existing, event, key);
    ...
    if (event.type === 'SALE_APPROVED') {
      ...
      // Si la limpieza ya libero una reserva vencida, volver a tomar esas
      // unidades antes de aceptar el pago. Si ya se vendieron a otra persona,
      // se registra la transaccion para revision sin crear sobreventa.
      const order = {
        ...baseOrder(reference, event, reservation),
