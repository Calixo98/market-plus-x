...
async function enviarEventoCompra({ referencia, total, moneda, email, telefono, ip, userAgent, eventTime }) {
  ...
  if (!token) {
    console.error('META_CAPI_ACCESS_TOKEN no configurado: no se envia el evento Purchase a Meta');
    return;
  }
  ...
  const body = {
    data: [{
      ...
      event_source_url: 'https://marketplusx.com/checkout.html',
      user_data: userData,
  ...
  try {
    const resp = await fetch(`https://graph.facebook.com/${GRAPH_API_VERSION}/${PIXEL_ID}/events?access_token=${token}`, {
      method: 'POST',
    ...
    if (!resp.ok) {
      console.error(`Meta rechazo el evento Purchase de ${referencia}:`, JSON.stringify(data));
    }
  } catch (err) {
    console.error(`Error de red enviando el evento Purchase de ${referencia}:`, err.message);
  }
}

module.exports = { enviarEventoCompra };
