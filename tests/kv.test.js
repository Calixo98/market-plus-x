...
test('SET de pedidos se envia por POST sin exponer la PII en la URL', async t => {
  const previousFetch = global.fetch;
  const previousUrl = process.env.KV_REST_API_URL;
  const previousToken = process.env.KV_REST_API_TOKEN;
  t.after(() => {
  ...
  delete require.cache[require.resolve('../lib/kv')];
  const kv = require('../lib/kv');
  await kv.set('pedido:MPX-DEMO', JSON.stringify({ direccion:'Calle 1 # 2-3', telefono:'573001234567' }));

  assert.equal(request.url, 'https://redis.example.test');
  assert.equal(request.options.method, 'POST');
  assert.match(request.options.body, /Calle 1/);
  assert.equal(request.url.includes('Calle'), false);
});
